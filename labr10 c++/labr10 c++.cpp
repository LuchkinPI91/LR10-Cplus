...
public:
	...
	students operator++(void) {// перегрухка ++a

		++a;
		return *this;
	}
	students operator++(int a) {//перегрузка a++
	...
	static int get_stipendia() { return stipendia; }//геттер для статического поля 

	void out_stipendia(int n) {// вывод наличия стипендии
	...
	float get_sum()// геттер  суммы баллов
	{
	...
	students& operator=(string str) {//перегрузка оператора присв-я
	...
	void get_str() {//геттер для строки str
